...
import { ALTEZZA_LAMPIONE } from '../src/ostacoli.js';
...
test('abbassandosi la testa passa sotto il lampione', () => {
  const corridore = creaCorridore();
  assertQuasi(altezzaTesta(corridore), ALTEZZA_OMINO, 1e-9);
  assert(altezzaTesta(corridore) > ALTEZZA_LAMPIONE, 'in piedi il lampione lo prende');

  scivola(corridore);
  assert(abbassato(corridore));
  assertQuasi(altezzaTesta(corridore), ALTEZZA_OMINO_ABBASSATO, 1e-9);
  assert(altezzaTesta(corridore) < ALTEZZA_LAMPIONE, 'abbassato ci passa sotto');
});
